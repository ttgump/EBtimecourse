...
library(tensorflow)
use_python("")
source("EBtimecourse_hotpoints.R")
...
W = 10
prob = rep(1, combNumber) + as.integer(changePointTable$n1 != 1 & changePointTable$n3 != 1)*(W-1)
prob = prob/sum(prob)
ss=sample(1:nrow(changePointTable), P*N, prob=prob, replace=T)

...
alpha=12; alpha0=1; v=36;
lambda=matrix(rgamma(N*2,shape=alpha0,rate=v),nc=2)
parameter=data.frame(lambda, n1=n1, n2=n2, n3=n3)
colnames(parameter)=c("lambda1", "lambda2", "n1", "n2", "n3")
gene.de=t(apply(parameter[1:nDE,],1, function(x) c(rgamma(x[3]*replicate,shape=alpha,rate=x[1]),
                                                   rgamma(x[4]*replicate,shape=alpha,rate=x[2]), 
                                                   rgamma(x[5]*replicate,shape=alpha,rate=x[1]))))
gene.ee=t(apply(parameter[(nDE+1):N,],1, function(x) c(rgamma(x[3]*replicate,shape=alpha,rate=x[1]))))
